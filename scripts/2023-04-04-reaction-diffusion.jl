...
prob_without_diffusion =
    ODEProblem{true,SciMLBase.FullSpecialize()}(f!, u0, tspan, [0.0, DX])

# Solve with Radau
radau_sol_with_diffusion = solve(prob_with_diffusion, RadauIIA3(), adaptive=false, dt=dt)
radau_sol_without_diffusion =
    solve(prob_without_diffusion, RadauIIA3(), adaptive=false, dt=dt)

plot(
    plot(
        radau_sol_with_diffusion,
        title="diffusion = 0.01",
        label=hcat(["x=$i" for i in 1:length(u0)]...),
    ),
    plot(
        radau_sol_without_diffusion,
        title="diffusion = 0.0",
...
plot(
    plot(
        EK1_sol_with_diffusion,
        title="diffusion = 0.01",
        label=hcat(["x=$i" for i in 1:length(u0)]...),
    ),
    plot(
        EK1_sol_without_diffusion,
        title="diffusion = 0.0",
        label=hcat(["x=$i" for i in 1:length(u0)]...),
    ),
)
...
EK1_ioup_sol_with_diffusion =
    solve(prob_with_diffusion, EK1(prior=IOUP(NU, laplace_1d(u0))), adaptive=false, dt=dt)
EK1_ioup_sol_without_diffusion = solve(
    prob_without_diffusion,
    EK1(prior=IOUP(NU, laplace_1d(u0))),
    adaptive=false,
    dt=dt,
)
...
plot(
    ...
    plot(
        EK1_ioup_sol_without_diffusion,
        title="diffusion = 0.0",
        label=hcat(["x=$i" for i in 1:length(u0)]...),
    ),
)
